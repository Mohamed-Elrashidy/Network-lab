...
def local_reset():
    reset_game()
    s.send(b'reset')
def reset_game(button_pressed=False):
    global player
    player = 1
...
reset_button = Button(wind, text="Reset", bg="red", fg="white", width=10, height=1, font=('Helvetica', '15'), command=local_reset)
...
def receive_message():
    while True:
        p = s.recv(10)
        if p == b'reset':  # handle reset signal
            reset_game()
        else:
            apply_play(p)
